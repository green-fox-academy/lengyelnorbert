...
handle = function handle(id, info, where, data) {
  ...
  $(id).append(
      "<li>" + info + (data ? " - check the console for the response " : "")
      + "</li>");
}
$("#doubling").on("click", function () {
  ...
  $.get(window.baseUrl + "/doubling", {input: 5}).done(function (data) {
    if ("received" in data && "result" in data && data["received"] == 5
        && data["result"] == 10) {
...
$("#greeter").on("click", function () {
  ...
  $.get(window.baseUrl + "/greeter", {name: "petike", title: "student"}).done(
      function (data) {
        if ("welcome_message" in data && data["welcome_message"]
            == "Oh, hi there petike, my dear student!") {
          handle("#greeter_response", "OK - with petike and student");
        } else {
          handle("#greeter_response", "not ok - with petike and student",
              "Greeter", data);
        }
      }).error(function (err) {
    handle("#greeter_response", "not ok - with petike and student", "Greeter",
        err);
  });
});
...
$("#dountil").on("click", function () {
  ...
  $.post({
    ...
    if ("result" in data && data["result"] == 28) {
      ...
      handle("#dountil_response", "not ok - with /sum until=7", "Dountil",
          data);
    }
  }).error(function (err) {
  ...
  $.post({
    ...
    if ("result" in data && data["result"] == 24) {
      ...
      handle("#dountil_response", "not ok - with /factor until=4", "Dountil",
          data);
    }
  }).error(function (err) {
    handle("#dountil_response", "not ok - with /factor until=4", "Dountil",
